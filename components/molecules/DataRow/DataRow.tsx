...
export function DataRow({ label, value, subValue, className }: DataRowProps) {
  return (
    <div
      className={cn(
        "flex flex-col sm:flex-row sm:items-center py-3 border-b border-slate-100 last:border-0 gap-1 sm:gap-4",
        className,
      )}
    >
      <div className="w-full sm:w-1/3 text-sm font-medium text-slate-700">{label}</div>
